...
class BaseEvaluator:
    ...
    #     if os.path.exists(fid_mean_cache_path) and os.path.exists(fid_cov_cache_path) and os.path.exists(
    #             jftsd_mean_cache_path) and os.path.exists(jftsd_cov_cache_path):
    ...
    #     else:
    #         train_loader = self.dataset.get_loader(split="train", batch_size=self.batch_size, shuffle=False,
    #                                                include_self=False)
        ...
        with torch.no_grad():
            for batch_no, batch in enumerate(self.test_loader):
                if hasattr(self, "long_clip_embeds_test"):
                    batch["cap_embed"] = self.long_clip_embeds_test["embeddings"][batch["indices"]]
                    batch["cap_embed_mask"] = self.long_clip_embeds_test["all_masks"][batch["indices"]]
