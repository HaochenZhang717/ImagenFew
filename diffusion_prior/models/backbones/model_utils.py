...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        freqs = broadcat((freqs_h[:, None, :], freqs_w[None, :, :]), dim=-1)
        freqs_cos = freqs.cos().reshape(ft_h * ft_w, -1)
        freqs_sin = freqs.sin().reshape(ft_h * ft_w, -1)
    ...
    def forward(self, t):
        ...
        assert Lt == L, f"Token length mismatch: input has {Lt}, rope has {L}"
