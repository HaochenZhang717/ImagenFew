"""
Export pretrained MultiScaleVAE posterior means for all finetune datasets listed in
configs/DiffusionTS/*.yaml.
...
def build_vae_model(vae_cfg, ckpt_path, device):
    model = DualVAE(
        z_channels=vae_cfg["z_channels"],
        ch=vae_cfg["unet_channels"],
...
def main():
    ...
    parser.add_argument("--vae-config", type=str, required=True, help="Path to VAE config, e.g. configs/pretrain/vae_pretrain.yaml")
    parser.add_argument("--vae-ckpt", type=str, required=True, help="Path to pretrained VAE checkpoint")
    parser.add_argument("--dataset-config-dir", type=str, required=True, help="Directory containing DiffusionTS finetune configs")
    ...
    model = build_vae_model(vae_cfg, args.vae_ckpt, args.device)
