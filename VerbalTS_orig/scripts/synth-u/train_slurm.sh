#!/usr/bin/env bash
#SBATCH --job-name=verbalts_synthu
...
export WANDB_NAME="${WANDB_NAME:-synth-u_text2ts_msmdiffmv}"

echo "Running VerbalTS synth-u training on host $(hostname)"
...
CMD=(
  ...
  --save_folder ./logs/synth-u/text2ts_msmdiffmv
  --model_diff_config_path configs/synth-u/diff/model_text2ts_dep.yaml
  --model_cond_config_path configs/synth-u/cond/text_msmdiffmv.yaml
  --train_config_path configs/synth-u/train.yaml
  --evaluate_config_path configs/synth-u/evaluate.yaml
