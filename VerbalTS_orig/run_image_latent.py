...
def update_image_diffusion_shape(model_diff_configs, data_folder, split="train"):
    ...
    if ts.ndim == 2:
        seq_len, n_var = ts.shape[1], 1
    elif ts.ndim == 3:
        seq_len, n_var = ts.shape[1], ts.shape[2]
    else:
        raise ValueError(f"Expected {ts_path} to have 2 or 3 dims, got shape={ts.shape}")

    diffusion_configs = model_diff_configs["diffusion"]
    diffusion_configs["seq_len"] = int(seq_len)
    diffusion_configs["n_var"] = int(n_var)
    if "side" in diffusion_configs:
        diffusion_configs["side"]["num_var"] = int(n_var)


def build_model(training_stage, model_diff_configs, model_cond_configs, dataset):
    if training_stage == "pretrain":
        return ImageUnConditionalGenerator(model_diff_configs)
    if training_stage == "finetune":
        ...
        return ImageConditionalGenerator(model_diff_configs, model_cond_configs)
...
def train(training_stage, train_configs, model_diff_configs, model_cond_configs, eval_configs, output_folder):
    ...
    update_image_diffusion_shape(model_diff_configs, train_configs["data"]["folder"], split="train")

    dataset = GenerationDataset(train_configs["data"])
    model = build_model(training_stage, model_diff_configs, model_cond_configs, dataset)
    ...
    print("Begin image-generator training!")
    pretrainer.train()
...
def evaluate(training_stage, eval_configs, model_diff_configs, model_cond_configs, output_folder):
    ...
    update_image_diffusion_shape(model_diff_configs, eval_configs["data"]["folder"], split="test")
    ...
    return _evaluate_cond_gen(evaluator)
...
parser = argparse.ArgumentParser(description="VerbalTS image generator")
...
parser.add_argument("--clip_cache_path", type=str, default="")

parser.add_argument("--cond_modal", type=str, default="text")
parser.add_argument("--text_output_type", type=str, default="segment")
...
model_diff_configs["diffusion"]["img_size"] = args.img_size
model_diff_configs["diffusion"]["patch_size"] = args.patch_size

if args.training_stage == "finetune" and model_cond_configs is not None and "text" in model_cond_configs:
    ...
    model_diff_configs["diffusion"]["condition_dim"] = model_cond_configs["text"]["text_emb"]
...
if args.clip_folder != "":
    ...
    if model_cond_configs is not None and model_cond_configs["cond_modal"] == "constraint":
