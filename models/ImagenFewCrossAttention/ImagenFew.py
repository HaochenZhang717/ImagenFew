...
class ImagenFew(nn.Module):
    def __init__(self, args, device):
        ...
        self.context_dim = getattr(args, "context_dim", None)
    ...
    def loss_fn(self, x, mask, class_indicies=None, num_classes=None, labels=None, context=None):
        '''
        ...
        if labels is None and class_indicies is not None and num_classes is not None:
            labels = nn.functional.one_hot(class_indicies, num_classes=num_classes)
        output, weight = self.forward(x * mask, labels=labels, context=context)

        # denoising matching term
        fft_x       = torch.fft.fft2(x, norm='forward')
        fft_output  = torch.fft.fft2(output, norm='forward')
        fft_loss    = (torch.real(fft_output) - torch.real(fft_x)).square() + (torch.imag(fft_output) - torch.imag(fft_x)).square()
        time_loss   = (output - x).square()
        loss = (weight * (time_loss + fft_loss) * mask).mean()
        to_log['karras loss'] = loss.detach().item()

        return loss, to_log

    def forward(self, x, mask, labels=None, context=None, augment_pipe=None):

        rnd_normal = torch.randn([x.shape[0], 1, 1, 1], device=x.device)
        sigma = (rnd_normal * self.P_std + self.P_mean).exp()
        # fft_sigma = (rnd_normal * self.P_std / np.sqrt(2*24) + self.P_mean).exp()
        weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data) ** 2
        # fft_weight = (fft_sigma ** 2 + self.sigma_data ** 2) / (fft_sigma * self.sigma_data) ** 2
        ...
        if context is not None:
            context = context.to(device=x.device, dtype=torch.float32)
        D_yn = self.net(y + x_to_impute, sigma, labels, augment_labels=augment_labels, context=context)
        return D_yn, weight

    def forward_forecast(self, past, future, labels=None, context=None, augment_pipe=None):
        ...
        if context is not None:
            context = context.to(device=past.device, dtype=torch.float32)
        D_yn = self.net(full_seq, sigma, labels, augment_labels=augment_labels, context=context)[..., s:(s + e)]
        return D_yn, weight
    ...
    def reset_ema(self):
        self.model_ema = LitEma(self.net, decay=0.9999, use_num_upates=True, warmup=self.args.ema_warmup)
