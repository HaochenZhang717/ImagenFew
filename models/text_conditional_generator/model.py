...
class SelfConditionalGenerator(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.text_embedding_dim = int(configs["text_embedding_dim"])
    ...
    # Training forward
    # ------------------------------------------------------------------
    def _prepare_text_context(self, text_context):
        if text_context is None:
            raise ValueError("text_context must be provided for text-conditional generation.")
        if text_context.ndim != 3:
            raise ValueError(
                f"Expected text_context with shape (B, n_var, D), got {tuple(text_context.shape)}"
            )
    ...
    def generate_from_context(self, context, seq_len, n_var, sampler="ddim"):
        context = self._prepare_text_context(context)
        B = context.shape[0]
    ...
    @torch.no_grad()
    def generate(self, n_samples, text_context, seq_len, n_var, sampler="ddim"):
        return self.generate_from_context(text_context, seq_len, n_var, sampler=sampler)
