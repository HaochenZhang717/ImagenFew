...
class Handler(generativeHandler):
    ...
    def build_model(self):
        ...
        if getattr(self.args, "pretrain_path", None):
            ...
            if model.use_ema:
                model.reset_ema()
            print(f"Loaded pretrained text-conditional diffusion model from {self.args.pretrain_path}")
    ...
    def train_iter(self, train_dataloader, logger):
        ...
        for batch_idx, (batch, class_indices) in enumerate(pbar, 1):
            if not isinstance(batch, (tuple, list)) or len(batch) != 2:
                raise ValueError(
                    "text_conditional_generator expects dataloader batches as (time_series, caption_embeddings)."
                )
            data, text_context = batch
            x = data.to(self.device).float()
            text_context = text_context.to(self.device).float()
            loss_dict = self.model(x, text_context, is_train=True)
            loss = loss_dict["all"]
    ...
    def _sample_with_posterior(self, n_samples, class_metadata, test_data):
        ...
        if test_data is None:
            raise ValueError("test_data must be provided for text-conditional sampling.")
        if not isinstance(test_data, (tuple, list)) or len(test_data) != 2:
            raise ValueError(
                "text_conditional_generator expects test_data as (time_series, caption_embeddings) during sampling."
            )
        _, caption_embeddings = test_data

        with self._model.ema_scope(), torch.no_grad():
            while remaining > 0:
                bs = min(batch_size, remaining)
                indices = torch.randperm(caption_embeddings.shape[0], device=caption_embeddings.device)[:bs]
                text_context = caption_embeddings[indices].to(device=self.device, dtype=torch.float32)
                samples = self._model.generate(bs, text_context, seq_len, n_var)
                generated.append(samples)
                remaining -= bs
    ...
    def _build_configs(self):
        ...
        text_embedding_dim = getattr(args, "text_embedding_dim", None)
        if text_embedding_dim is None:
            caption_embeddings_path = getattr(args, "caption_embeddings_path", None)
            if caption_embeddings_path is None:
                raise ValueError("text_embedding_dim or caption_embeddings_path must be provided for text_conditional_generator.")
            caption_embeddings = torch.load(caption_embeddings_path, map_location="cpu", weights_only=False)
            if not torch.is_tensor(caption_embeddings) or caption_embeddings.ndim != 3:
                raise ValueError(
                    f"caption_embeddings_path must point to a tensor of shape (N, C, D), got {type(caption_embeddings)}"
                )
            text_embedding_dim = int(caption_embeddings.shape[-1])
