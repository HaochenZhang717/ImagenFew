...
class DiffusionProcess():
    ...
    @torch.no_grad()
    def sampling(self, sampling_number=16, impute=False, xT=None,class_labels=None):
        if xT is None:
            xT = torch.randn([sampling_number, *self.shape]).to(device=self.device)
        return self.sample(xT,class_labels = class_labels)


    @torch.no_grad()
    def interpolate(self, x, mask, class_labels, xT=None):
        if xT is None:
            xT = torch.randn([x.shape[0], *self.shape]).to(device=self.device)

        return self.impute(x, xT, mask, class_labels=class_labels)
