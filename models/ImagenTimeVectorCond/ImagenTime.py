...
class ImagenTime(nn.Module):
    ...
    def loss_fn(self, x, condition_vectors=None):
        '''
        ...
        output, weight = self.forward(x, labels=condition_vectors)
    ...
    def loss_fn_impute(self, x, mask, condition_vectors=None):
        '''
        x          : real data if idx==None else perturbation data
        idx        : if None (training phase), we perturbed random index.
        '''
        ...
        to_log = {}
        output, time_weight = self.forward_impute(x, mask, labels=condition_vectors)
