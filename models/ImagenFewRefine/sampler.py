...
class DiffusionProcess():
    ...
    def sample(self, latents, mask, class_labels=None, context=None):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.net(x_hat*mask, t_hat, class_labels, context=context).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < self.num_steps - 1:
                denoised = self.net(x_next*mask, t_next, class_labels, context=context).to(torch.float64)
                d_prime = (x_next - denoised) / t_next
                x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)

        return x_next

    def impute(self, x, latents, mask, class_labels=None, context=None):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            # Euler step.
            denoised = self.net(x_hat, t_hat, class_labels, context=context).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            ...
            # Apply 2nd order correction.
            if i < self.num_steps - 1:
                denoised = self.net(x_next, t_next, class_labels, context=context).to(torch.float64)
                d_prime = (x_next - denoised) / t_next
    ...
    def forecast(self, past, latents, pad, f_shape, class_labels=None, context=None):
    ...
    @torch.no_grad()
    def sampling(self, mask, sampling_number=16, impute=False, xT=None, class_labels=None, context=None):
        if xT is None:
            xT = torch.randn([sampling_number, *self.shape]).to(device=self.device)
        return self.sample(xT, mask, class_labels=class_labels, context=context)


    @torch.no_grad()
    def interpolate(self, x, mask, class_labels=None, context=None, xT=None):
        if xT is None:
            xT = torch.randn([x.shape[0], *self.shape]).to(device=self.device)

        return self.impute(x, xT, mask, class_labels=class_labels, context=context)


    @torch.no_grad()
    def forecasting(self, x, pad, f_shape, xT=None, class_labels=None, context=None):
        if xT is None:
            xT = torch.randn([x.shape[0], *self.shape]).to(device=self.device)

        return self.forecast(x, xT, pad, f_shape, class_labels=class_labels, context=context)
