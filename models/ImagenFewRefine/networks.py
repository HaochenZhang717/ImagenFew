...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.head_channels = out_channels // self.num_heads if self.num_heads else 0
        self.dropout = dropout
        self.skip_scale = skip_scale
        self.adaptive_scale = adaptive_scale
        self.context_dim = context_dim
        self.has_cross_attention = self.num_heads > 0 and context_dim is not None
        ...
        if self.num_heads:
            ...
            if r is not None:
                self.qkv = lora.Conv2d(in_channels = out_channels, out_channels = out_channels*3, r = r, kernel_size = 1)
                self.proj = lora.Conv2d(in_channels = out_channels, out_channels = out_channels, r = r, kernel_size = 1)
            else:
                self.qkv = Conv2d(in_channels=out_channels, out_channels=out_channels*3, kernel=1, **(init_attn if init_attn is not None else init))
                self.proj = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=1, **init_zero)
            if self.has_cross_attention:
                self.norm3 = GroupNorm(num_channels=out_channels, eps=eps)
                self.cross_q = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=1, **(init_attn if init_attn is not None else init))
                self.cross_k = Linear(in_features=context_dim, out_features=out_channels, **(init_attn if init_attn is not None else init))
                self.cross_v = Linear(in_features=context_dim, out_features=out_channels, **(init_attn if init_attn is not None else init))
                self.cross_proj = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=1, **init_zero)
    ...
    def forward(self, x, emb, context=None):
        ...
        if self.num_heads:
            ...
            if self.has_cross_attention and context is not None:
                q = self.cross_q(self.norm3(x)).reshape(x.shape[0], self.num_heads, self.head_channels, -1)
                context = context.to(x.dtype)
                k = self.cross_k(context).reshape(context.shape[0], context.shape[1], self.num_heads, self.head_channels)
                v = self.cross_v(context).reshape(context.shape[0], context.shape[1], self.num_heads, self.head_channels)
                q = q.reshape(x.shape[0] * self.num_heads, self.head_channels, -1)
                k = k.permute(0, 2, 3, 1).reshape(x.shape[0] * self.num_heads, self.head_channels, context.shape[1])
                v = v.permute(0, 2, 3, 1).reshape(x.shape[0] * self.num_heads, self.head_channels, context.shape[1])

                w = AttentionOp.apply(q, k)
                a = torch.einsum('nqk,nck->ncq', w, v)
                x = self.cross_proj(a.reshape(*x.shape)).add_(x)
                x = x * self.skip_scale
        return x
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        context_dim         = None,         # Context token dimensionality for cross-attention.
    ):
        ...
        self.context_proj_dim = model_channels if context_dim is not None else None
        self.context_projector = (
            ContextProjector1D(context_dim, model_channels, self.context_proj_dim)
            if context_dim is not None else None
        )
        block_kwargs = dict(
            emb_channels=emb_channels, num_heads=1, dropout=dropout, skip_scale=np.sqrt(0.5), eps=1e-6,
            resample_filter=resample_filter, resample_proj=True, adaptive_scale=False,
            init=init, init_zero=init_zero, init_attn=init_attn, context_dim=self.context_proj_dim,
        )
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, context=None):
        # Mapping.
        ...
        if context is not None and self.context_projector is not None:
            context = self.context_projector(context.to(torch.float32)).to(x.dtype)
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        block_kwargs = dict(
            ...
            init_zero=init_zero,
            r=lora_rank,
            context_dim=self.context_proj_dim,
        )
        self.dynamic_size = dynamic_size
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels * mult
                self.enc[f'{res}x{res}_conv'] = DynamicConv2d(dynamic_size=self.dynamic_size, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res}x{res}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
            for idx in range(num_blocks):
                cin = cout
        ...
        self.out_conv = DynamicConv2d(dynamic_size=self.dynamic_size, kernel=3, **init_zero)
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None, context=None):
        ...
        for block in self.dec.values():
            if x.shape[1] != block.in_channels:
                x = torch.cat([x, skips.pop()], dim=1)
            x = block(x, emb, context=context)
        x = self.out_conv(silu(self.out_norm(x)), C)
        return x
