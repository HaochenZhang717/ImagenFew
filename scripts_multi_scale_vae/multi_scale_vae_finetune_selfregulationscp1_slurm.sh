#!/usr/bin/env bash
#SBATCH --job-name=msvae_selfregulationscp1
...
CONFIG="${CONFIG:-$ROOT_DIR/configs/multi_scale_vae/SelfRegulationSCP1.yaml}"
WANDB_PROJECT="${WANDB_PROJECT:-MultiScaleVAEFinetune}"
...
echo "Running MultiScaleVAE finetune-from-scratch on host $(hostname)"
...
python run_no_sample.py   --seed "$SEED"   --subset_p "$SUBSET_P"   --wandb   --wandb_project "$WANDB_PROJECT"   --config "$CONFIG"   "$@"
