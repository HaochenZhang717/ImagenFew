...
def main(args):
    ...
    n = 128 // args.num_channels
    num_batches = math.ceil(num_samples / n)

    samples_list = []
    with open(args.output_jsonl, "w") as f:
        for batch_id in tqdm(range(num_batches)):
            z = torch.randn(n, *tuple(args.latent_size), device=device)
            n, D, CH, W = z.shape
            C = args.num_channels
            H = CH//args.num_channels
            # z = z.reshape(B_ts, D, C*H, W)


            # z = z.reshape(n, D, C * H, W)
            # set guidance setup
            model_kwargs = dict()
            model_fwd = model.forward
            # Sample images:
            samples:torch.Tensor = sample_fn(z, model_fwd, **model_kwargs)[-1]
            samples = samples.contiguous().view(n, 1152, C, H, W).permute(2,0,1,3,4) # (C, n, 1152, H, W)

            #
            # for channel in range(C):
            #     start_time = time()
            #     channel_samples = samples[channel] # (n, 1152, H, W)
            #     channel_samples = channel_samples.contiguous().view(n, 1152, -1).permute(0, 2, 1).contiguous().view(-1, 1152)
            #     decoded_samples = vlm.decode(channel_samples, tokens_per_sample=H*W)
            ...
            for channel in range(C):
                ...
                decoded_samples = vlm.decode(channel_samples, tokens_per_sample=H * W)
                print(f"Decode channel {channel} took {time() - start_time:.2f} seconds.")
                all_decoded.append(decoded_samples)
            ...
            for i in range(n):
                ...
                captions = []
                for channel in range(C):
                    text = all_decoded[channel][i]
                    lines = text.strip().split("\n")

                    for seg_id, line in enumerate(lines):
                        seg_text = line.split(":", 1)[1].strip()
                        key = f"seg{seg_id + 1}_channel{channel}"
                        captions.append({key: seg_text})

                record = {
                    "id": f"image{batch_id * n + i}",
                    "captions": captions
                }
                f.write(json.dumps(record) + "\n")
                samples_list.append(captions)
            f.flush()
