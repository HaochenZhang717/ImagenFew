...
def save_worktree(path: str, config: OmegaConf) -> None:
    OmegaConf.save(config, os.path.join(path, "config.yaml"))
    repo_root = os.path.dirname(os.path.abspath(__file__))  # utils/.. 的位置可能还要调
    repo_root = os.path.abspath(os.path.join(repo_root, ".."))
    # shutil.copytree(repo_root, os.path.join(path, "src/"), dirs_exist_ok=True)
    shutil.copytree(
        repo_root,
        os.path.join(path, "src/"),
        dirs_exist_ok=True,
        ignore=shutil.ignore_patterns("wandb", "ckpts", "__pycache__", ".git")
    )

    print(f"Worktree {repo_root} saved to {os.path.join(path, 'src/')}")
